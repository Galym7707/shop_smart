...
import { useAuth } from '../context/AuthContext'; // Змінено на відносний шлях
...
export default function ShoppingList({ uuid }: ShoppingListProps) {
    ...
    useEffect(() => {
        if (!authLoading && !isAuthenticated) {
            // router.push('/login'); // Розкоментуй, якщо список завжди приватний
        }
    }, [authLoading, isAuthenticated, router]);

    useEffect(() => {
        if (!uuid || !API_BASE_URL) return; // Додано перевірку API_BASE_URL
    ...
    }, [uuid, token, API_BASE_URL]);

    const handleAddItem = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (!newItemName.trim() || !API_BASE_URL) return;
        if (!isAuthenticated) {
                    ...
                    {items.length === 0 && !loading ? (
                        ...
                        <ul className="space-y-3">
                            {items.map((item) => (
                                <li
                                    key={item._id}
                                    className={`flex justify-between items-center p-3 rounded-md transition-all duration-200 ease-in-out group ${item.bought ? 'bg-green-50 dark:bg-green-900/30 opacity-70' : 'bg-gray-50 dark:bg-gray-700/60 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                                >
                                    <span 
                                    ...
                                    {isAuthenticated && (
                                        <div className="flex items-center gap-2 ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                            <button onClick={() => handleToggleBought(item._id)} title={item.bought ? 'Mark as not bought' : 'Mark as bought'} className={`p-1.5 rounded-full transition-colors ${item.bought ? 'text-yellow-600 hover:bg-yellow-200 dark:hover:bg-yellow-700' : 'text-green-600 hover:bg-green-200 dark:hover:bg-green-700'}`}>
                                                {item.bought ? '↩️' : '✔️'} 
                                            </button>
                                            <button onClick={() => handleDeleteItem(item._id)} title="Delete item" className="text-red-500 hover:bg-red-200 dark:hover:bg-red-700 p-1.5 rounded-full transition-colors">
                                                🗑️
                                            </button>
                                        </div>
                                    )}
